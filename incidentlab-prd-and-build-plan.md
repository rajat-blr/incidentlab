...
- **Step 7 complete.** OpenAI is isolated behind a typed adapter and a versioned Structured Outputs prompt. Deterministic code limits artifact follow-ups to two allowlisted, evidence-kind-specific lookups; validates every schema and citation; rejects gaps as supporting evidence; redacts likely secrets; and records hypotheses, model usage, tool calls, and rejections. Adversarial tests prove that a log-line prompt injection cannot introduce a shell tool and an invented citation is rejected. The repeatable `scripts/step7_diagnosis_check.py` live acceptance run produces evidence-grounded hypotheses, resolves every citation, uses no gap as supporting evidence, records model provenance and the diagnosis audit event, and closes cleanly after repair rejection.
