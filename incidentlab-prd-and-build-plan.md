...
- **Step 5 complete.** `POST /runs` starts one durable Temporal workflow per idempotency key. Activities reproduce the incident, persist state and audit events, emit deterministic placeholder collection/diagnosis/repair/verification/report outputs, wait for repair approval, and handle cancellation. Activities use stable effect keys, bounded retries and timeouts; reproduction and verification use heartbeats. The repeatable `scripts/step5_restart_check.py` acceptance check created the same run twice, killed the worker at the approval gate, recorded approval while it was down, restarted it, and reached `COMPLETED` with one effect per state. A separate check reached `CANCELLED` and repeated cancellation without another effect. Runs pin the repository's real Git commit.
- **Step 6 complete.** The workflow emits a run-scoped incident window and bounded collectors query Jaeger, Prometheus, Loki, and Git metadata. Exact responses are stored as immutable, SHA-256-addressed artifacts; normalized facts and explicit gaps cite those artifacts. Unit tests cover empty, late, duplicate, and malformed telemetry. The Compose acceptance check produced one trace, three metric, two log, and one commit fact with zero gaps, and re-hashed every artifact served by the API. A fresh PostgreSQL database passed migration upgrade and downgrade, and `alembic check` found no schema drift.
- **Step 7 complete.** Gemini is isolated behind a typed adapter and a versioned structured-output prompt. Deterministic code limits artifact follow-ups to two allowlisted, evidence-kind-specific lookups; validates every schema and citation; rejects gaps as supporting evidence; redacts likely secrets; and records hypotheses, model usage, tool calls, and rejections. Adversarial tests prove that a log-line prompt injection cannot introduce a shell tool and an invented citation is rejected. The repeatable `scripts/step7_diagnosis_check.py` live acceptance run used `gemini-3.5-flash-lite`, produced one hypothesis from seven saved evidence items, resolved every citation, used no gap as supporting evidence, recorded model provenance and the diagnosis audit event, and closed cleanly after repair rejection.
- **Step 8 complete.** A host-side runner exports fresh baseline and candidate trees at a full pinned commit, excludes private evaluation truth, validates and applies a bounded manual diff, and runs fixed checks in a pinned Docker image. Containers are non-root, network-disabled, read-only, capability-free, `no-new-privileges`, and constrained by CPU, memory, PID, file-descriptor, tmpfs, wall-time, and output limits. Logs and image identity are retained with SHA-256 hashes while temporary workspaces and containers are cleaned up. `scripts/step8_sandbox_check.py` proved the baseline incident, passed a known-good repair through build/static/unit/integration/five replay checks, rejected a syntax-broken repair at build, rejected traversal before execution, and contained a hostile probe from the Docker socket, repository metadata, external network, root filesystem, and candidate mount. The boundary and its limitations are documented in `docs/sandbox.md`.
- **Step 9 complete.** Repair generation requires a recorded approval and receives only the selected hypothesis, failing reproduction, and exact allowlisted source blob from the pinned commit. Gemini returns at most two bounded line replacements; trusted code derives unified diffs, and deterministic policy rejects forbidden paths, file operations, oversized changes, secrets, unsafe execution primitives, invalid syntax, and patches that Git cannot apply cleanly. Accepted candidates and model usage are persisted with hashes and policy provenance. `scripts/step9_repair_check.py` proves that no candidate exists before approval, generates an accepted candidate, verifies its hash and audit order, passes it through the independent Step 8 sandbox, and cancels cleanly at the Step 10 persistence handoff.
- **Step 10 complete.** The workflow now pauses durably in `VERIFYING` while a trusted host-side process, kept separate from the API and model worker, runs each accepted candidate in the Step 8 sandbox. Baseline reproduction is mandatory before post-patch build, static, unit, integration, and five-trial incident replay checks. Check logs are persisted as SHA-256-addressed artifacts; outcomes are derived from mandatory facts, infrastructure interruptions become `INCONCLUSIVE`, and `verification-score-v1` ranks candidates repeatably by mandatory outcome and diff complexity. `scripts/step10_verification_check.py` persisted and re-hashed seven raw artifacts, ranked one candidate `PASS`, and advanced the signaled workflow to `COMPLETED`. Unit tests prove that a missing baseline failure cannot pass, persistent post-patch failure is disqualifying, and ranking is independent of input order.
- **Step 11 complete.** A responsive React and TypeScript review console now lists and filters durable runs, follows active state with bounded polling, and presents the workflow timeline, raw evidence, cited hypotheses, approval controls, escaped unified diffs, policy decisions, ranked verification checks and logs, and the immutable audit trail. FastAPI exposes run listing and versioned JSON/Markdown report export; nginx provides same-origin API proxying and SPA fallback in Compose. Component tests prove durable API state rendering and safe treatment of model-provided diff markup. Browser acceptance against a completed run confirmed that a fresh reviewer can trace the diagnosis to seven evidence items, inspect the accepted patch, verify all seven mandatory checks, and download the report without using backend tools.
- **Step 12 complete.** The independent `inventory-underflow` scenario bypasses the insufficient-stock guard and commits a negative quantity, while reset plus the healthy control rejects the same request. `scripts/evaluate.py` repeatedly executes both versioned scenarios without model quota, compares the pipeline oracle with deterministic-keyword and uncited one-shot baselines, runs six adversarial probes (worker termination, timeout, duplicate request, malformed output, poisoned logs, and flakiness), and exports a dataset digest, JSON report, CSV metrics, and explicit failure report. The documentation distinguishes repeatable regression results from live-model quality claims.
- **Step 13 complete.** The optional GitHub boundary validates webhook HMAC-SHA256 signatures and requires only metadata read, contents write, and pull-request write permissions while rejecting deployment, workflow, or administration write access. A second durable approval, verified `PASS` candidate, deterministic branch, and existing-PR lookup guard the explicit CLI operation. Tests prove retries create one draft and an unverified candidate produces no write. No merge, ready-for-review, deployment, or workflow operation exists in the client.
- **Step 14 complete.** The README now covers clean setup, both scenarios, evaluation, reset, review, and optional integration. Architecture and trust-boundary diagrams, threat model, failure/recovery analysis, evaluation methodology, sandbox documentation, and two ADRs explain the system and its tradeoffs. `scripts/demo_release.py` demonstrates both deliberate failures, clean-reset recovery controls, adversarial containment, and generated evaluation artifacts without a provider key.

The first-release build plan is complete. Further work is outside this PRD's definition of done.
